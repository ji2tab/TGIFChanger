...
- **配置ディレクトリ変更**: `/opt/tgifchanger/` に変更し、`/usr/local/bin/tg_change` シンボリックリンクで PATH 上から直接実行可能に
...
1. `/opt/tgifchanger/` への各スクリプト配置
2. `/etc/tgifchanger.conf` の配置（既存ファイルは保護、新版は `.dist` として保存）
3. `/usr/local/bin/tg_change` シンボリックリンク作成（PATH 上から直接実行可能に）
...
シンボリックリンクが作成されているため、PATH 上から直接実行可能です:
...
```text
/opt/tgifchanger/tg_change
/opt/tgifchanger/auto_tg_restore
/opt/tgifchanger/log_monitor

/usr/local/bin/tg_change           → /opt/tgifchanger/tg_change (symlink)
