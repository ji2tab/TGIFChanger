...
**Version v2.3.4 (Python 3 Unified Daemon & Smart Installer Edition)**
...
さらに v2.3.4 では **コールサイン・ウォッチドッグ（真の利用者監視）** を搭載しました。他TGへ実際に RF アクセスした最後の局（＝真の利用者）を追跡し、その局の RF が一定時間（デフォルト 300秒）確認できなくなった場合は、ネット側（リモート）の通話が続いていても強制的にホームTGへ復帰します。これにより、遠方局同士のラグチューがネット越しに延々と流れ続けて自局の待ち受けが奪われ続ける、といった事態を防ぎます。

---

## 🚀 v2.3.4 の進化ポイント (旧シェルスクリプト版からの改善)
...
- 従来の Pi-Star (Buster) から最新の Bookworm 環境まで、Raspberry Pi Zero 〜 4 の幅広い環境で安定してGPIOピンを駆動します。
...
### 5. コールサイン・ウォッチドッグ (真の利用者監視) ［v2.3.4 新機能］

- 他TGへ **実際に RF（自局側）でキーアップした最後の局** を「真の利用者」として記録・追跡します。
- その局の RF が `CALLSIGN_TIMEOUT` 秒（デフォルト 300秒）確認できなければ、**ネット側（リモート）の通話が続いていても** `RESTORE_TG` へ強制復帰します。network 受信（リモート通話）はカウント対象外です。
- 別の局が RF キーアップすると、追跡対象をその局へ自動で切り替えます。
- `CALLSIGN_TIMEOUT="0"` を指定すると本機能は無効化され、従来どおり `RESTORE_DELAY` のみで動作します。
...
新規インストールの場合、インストール中にプロンプトが一時停止し、「監視TG」「復帰TG」「復帰までの時間」「コールサイン監視時間」 を対話形式で入力できます。
