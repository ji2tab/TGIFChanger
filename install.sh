...
#     両方を削除するクリーンアップを維持
#   - [FIX] BASH_SOURCE[0] unbound variable エラーの回避とGitHub自動フォールバック
#   - [NEW] 新規インストール時の対話型プロンプト(監視TG/復帰TG/復帰時間)の追加
# =============================================================================
...
# ------------------------------------------------------------------
# 1. 旧バージョンのクリーンアップ
#    - v2.x (tgifchanger-py サービス / /opt/tgifchanger-py)
...
# 旧ディレクトリ (proto-1.0.0 は /opt/tgifchanger, v2.x は /opt/tgifchanger-py)
...
# 引数で指定されたか、実行した場所にローカルファイルが存在しない場合はGitHubから取得
if [[ "${1:-}" == "--from-github" ]] || [[ ! -f "./tgif_daemon.py" ]]; then
    ...
    # ローカルファイルからインストール (BASH_SOURCE未定義エラー回避済)
...
EOF
    ...
    # パイプ実行時 (/dev/tty) でも入力を受け付ける対話型プロンプト
    if [ -t 0 ] || [ -c /dev/tty ]; then
        echo ""
        echo "💡 動作設定を入力してください（そのままEnterを押すとデフォルト値が採用されます）"
        
        read -p "▶ 監視TG (WATCH_TG) [デフォルト: 1]: " INPUT_WATCH_TG </dev/tty || true
        WATCH_TG="${INPUT_WATCH_TG:-1}"

        read -p "▶ 復帰TG (RESTORE_TG) [デフォルト: 168]: " INPUT_RESTORE_TG </dev/tty || true
        RESTORE_TG="${INPUT_RESTORE_TG:-168}"

        read -p "▶ 復帰までの時間(秒) (RESTORE_DELAY) [デフォルト: 120]: " INPUT_RESTORE_DELAY </dev/tty || true
        RESTORE_DELAY="${INPUT_RESTORE_DELAY:-120}"
        echo ""
    else
        echo "⚠️ 非対話モードで実行されています。デフォルト値を採用します。"
        WATCH_TG="1"
        RESTORE_TG="168"
        RESTORE_DELAY="120"
    fi

    # 変数を展開させるためクォートなしの EOF を使用
    cat > "${CONF_FILE}" <<EOF
# TGIFChanger 設定ファイル (v2.3.0)
#
# Bash KEY=VALUE 形式。両方の環境で読み込み可能:
