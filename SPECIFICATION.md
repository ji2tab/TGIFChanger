...
GPIO17 への HIGH 出力は、すべての受信イベントで発生するわけではない。以下の条件をすべて満たした受信ログエントリのみをトリガーとする。

1. **スロット一致:** ログに記録されたタイムスロットが `WATCH_SLOT` の設定値と一致すること。
2. **TG一致:** 受信したトークグループ番号が `WATCH_TG` の設定値と一致すること。
