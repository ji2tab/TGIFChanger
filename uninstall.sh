...
#
# Changes from v2.3.1:
#   - 設定ファイル削除に tg_change の排他ロック/一時ファイル
#     (.conf.lock / .conf.tmp) を追加し、消し残りを防止。
# =============================================================================
set -euo pipefail
echo "=================================================="
...
# 5. 設定ファイルの完全削除 (tg_change のロック/一時ファイルも含む)
echo "🗑️  設定ファイルを完全に削除しています..."
rm -f /etc/tgifchanger.conf \
      /etc/tgifchanger.conf.new \
      /etc/tgifchanger.conf.lock \
      /etc/tgifchanger.conf.tmp
echo "=================================================="
